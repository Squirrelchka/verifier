...
describe('hasAnyAuthority', () => {
  ...
  it('Should return false when authorities is invalid', () => {
    ...
    expect(hasAnyAuthority([], [AUTHORITIES.USER_STUDENT])).toEqual(false);
  });

  it('Should return true when authorities is valid and hasAnyAuthorities is empty', () => {
    expect(hasAnyAuthority([AUTHORITIES.USER_STUDENT], [])).toEqual(true);
  });

  it('Should return true when authorities is valid and hasAnyAuthorities contains an authority', () => {
    expect(hasAnyAuthority([AUTHORITIES.USER_STUDENT], [AUTHORITIES.USER_STUDENT])).toEqual(true);
    expect(hasAnyAuthority([AUTHORITIES.USER_STUDENT, AUTHORITIES.ADMIN], [AUTHORITIES.USER_STUDENT])).toEqual(true);
    expect(hasAnyAuthority([AUTHORITIES.USER_STUDENT, AUTHORITIES.ADMIN], [AUTHORITIES.USER_STUDENT, AUTHORITIES.ADMIN])).toEqual(true);
    expect(hasAnyAuthority([AUTHORITIES.USER_STUDENT, AUTHORITIES.ADMIN], [AUTHORITIES.USER_STUDENT, 'ROLEADMIN'])).toEqual(true);
    expect(hasAnyAuthority([AUTHORITIES.USER_STUDENT, AUTHORITIES.ADMIN], [AUTHORITIES.ADMIN])).toEqual(true);
  });

  it('Should return false when authorities is valid and hasAnyAuthorities does not contain an authority', () => {
    expect(hasAnyAuthority([AUTHORITIES.USER_STUDENT], [AUTHORITIES.ADMIN])).toEqual(false);
    expect(hasAnyAuthority([AUTHORITIES.USER_STUDENT, AUTHORITIES.ADMIN], ['ROLE_USER_STUDENTSS'])).toEqual(false);
    expect(hasAnyAuthority([AUTHORITIES.USER_STUDENT, AUTHORITIES.ADMIN], ['ROLEUSER', 'ROLEADMIN'])).toEqual(false);
  });
});
